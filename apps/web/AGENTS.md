...
Decided under CBD-20. The landing page (`/`) and the Mission page (`/mission`) live in the `(public)` route group with a shared shell, and every customer-readable sentence on them is an approved string from `docs/brand-foundation.md`, read at build time by `src/content/brand-foundation.ts` and rendered — never copied into a component. Changing wording means changing that document first. Headings, navigation labels, and chrome are structure, not copy, and are the only text a page adds. `scripts/check-public-pages.mjs` runs after the build in `npm run check`: it reads the document independently, requires every approved string in the built HTML in the required order with the closing line as the last text in `<main>`, scans the remainder for shame, fear, surveillance, real-time, and the two role names CBD-12 has not settled, and fails on any resource from another origin — a public page loads nothing external, which is what makes an analytics SDK, pixel, or session replay impossible rather than merely absent (`AN-92-001`, `AN-92-002`). `/foundation` stays outside the public group on purpose.
