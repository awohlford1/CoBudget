...
**Route B taken as well, September 2, 2026.** Twelve days after route A was
authorised the observation pass had not been run, and the selection was what
other work waited on. The Product Owner therefore also took route B: a
candidate at `ELIGIBLE-PENDING-EVIDENCE` may be selected, with the ten tests
deferred to build. **Route A is not withdrawn** — the observations remain
authorised and owed. The risk this section stated when it offered route B is
accepted rather than retired. Recorded at evidence register §3.3.1 and applied
at CBD-108's disposition register §4.
...
| HG-102-013 contractual backup behaviour | DOC | `UNPROVEN` | **`FAIL`** | **`FAIL`** | **Re-measured at v1.4 and corrected at v1.6 for C1.** **C1's v1.4 `FAIL` cited the region limb, and that limb is satisfied.** Seven of the eight components are on the data-residency list, and CBD-108 §4.51 establishes from `TD-103-004` and `TD-103-006` that the eighth, **Cloud Scheduler, holds no CoBudget data** — the tick enqueues, and the queue payload carrying `tenant_id`, `resource_ref` and `delegated_subject` is Pub/Sub, which is on the list. A commitment's failure to reach a component that holds nothing costs nothing. **The live limb for C1 is lifetime, which v1.4 did not cite.** Catalog §12.1's v1.3 record anticipated it: extending the test to routine expiry *"moves C1's lifetime element from met to not met"*, because Cloud Logging states no routine-expiry timing. **That is `UNPROVEN` and not `FAIL`** — the absence sits on one page, and an absence on a page is not established provider silence — the distinction CBD-108 preserves by registering failed retrievals as negative records rather than as findings. **C2 and C3 are unchanged.** C2 fails on lifetime, on CloudWatch Logs stating a bound with an unbounded tail, and C3 on region, Front Door being absent from a Core Services list that rendered in full — though CBD-108 `OI-108-046` records that C3's is configuration-dependent in a way C1's was not. |
...
| `UNPROVEN` | 19 | 19 | 19 |
| `FAIL` | 0 | 1 | 1 |
| **Verdict** | `ELIGIBLE-PENDING-EVIDENCE` | `INELIGIBLE` | `INELIGIBLE` |
