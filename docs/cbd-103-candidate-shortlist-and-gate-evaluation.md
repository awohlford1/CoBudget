...
| HG-102-013 contractual backup behaviour | DOC | **`FAIL`** | **`FAIL`** | **`FAIL`** | **Re-measured against v1.3 on August 30, 2026, and all three now fail.** The v1.1 position held that *"Silence fails"* means the provider's silence **once its contract has been read**, and that none of the three was yet provider silence. **The contracts have since been read** — all three DPAs, the AWS Customer Agreement, and the per-service documentation two of them defer to — so that defence is spent. Each candidate now fails on a **named component of its own composition**, which is what v1.3's scoping requires. **C1**: the current data-residency list, supplied complete, **omits Cloud Scheduler**, on a list the Product Owner supplied complete (CBD-108 §4.11), so the region element is absent for a required component. **C2**: CloudWatch Logs deletes *"up to 72 hours"* after the configured retention *"but in rare situations might take longer"* (CBD-108 §4.30) — a stated bound with an unbounded tail, which v1.3 fails in terms. **C3**: **Azure Front Door is absent** from the Microsoft Azure Core Services list, checked against a list that rendered in full (CBD-108 §4.26), so the Product Terms commitment CBD-108 §4.19 records does not reach the C3 edge. **These are `FAIL` and not `UNPROVEN` because register §3.3 reserves `FAIL` for *"evidence shows the property is absent"***, and in each case a document that rendered completely shows it. `OQ-103-024` warned this gate could fail every candidate at once; it has. **Qualified at v1.5: C1's and C3's failures are on the **region** limb and rest on an unexamined premise.** The gate governs copies of CoBudget data, and whether Cloud Scheduler and Azure Front Door hold any at rest has never been established — CBD-108 `OQ-108-061`. If they hold none, the commitment's failure to reach them is vacuous and register §3.3 would make the outcome `UNPROVEN` rather than `FAIL` (`OQ-108-062`). **C2's failure is unaffected**, being on the lifetime limb and on a component that demonstrably stores data. |
...
| `UNPROVEN` | 18 | 19 | 19 |
| `FAIL` | 1 | 1 | 1 |
| **Verdict** | `INELIGIBLE` | `INELIGIBLE` | `INELIGIBLE` |
