...
| Release gates carried | `RG-94-007` and `RG-94-012` gate external-notification implementation and copy under `FU-95-001`; neither closes through this specification. Tracked here as `OI-74-003` for copy and `OI-74-005` for provider and template approval. |
...
| Firm alert | An alert describing a settled fact. `SD-071-043` names the firm categories; the properties that it supports acknowledgement and does not self-clear come from CBD-72 §5.4.1 item 6 and CBD-71 §8A. |
| Informational alert | An alert describing a provisional outcome. It never asserts that a limit was exceeded and self-clears when its source resolves (`SD-071-043`). That it exposes **no acknowledgement operation** is the stronger CBD-72 permission 13 rule, not `SD-071-043`, which says only that none is required. |
...
| ID | Invariant | Source |
| --- | --- | --- |
| AB-74-001 | The built-in category set in §4.1 is closed. No member, preference, role, or configuration may create, author, rename, retire, or add a category, trigger, threshold, cooldown, or deduplication rule. Those are fixed product behavior. | `SD-071-044` for the category and trigger set; CBD-72 §5.4 and CBD-71 §8A for fixed thresholds and system-owned cooldown and deduplication, which `SD-071-044` does not name; `DD-071-009`; CBD-74-AC02; `FU-95-002` |
...
| AB-74-003 | Eligibility is evaluated per recipient against current membership, role, consent, and resource visibility at event creation, at material revision, on any membership, role, profile, scope-group, or consent change while the event is active, at rendering, at send time, and at protected-detail open time. Rendering and send are separate checks under `SR-94-048`, and deferment may separate them by hours. Preference never widens eligibility, and eligibility never depends on another member's settings. | CBD-72 `PM-72-002`; §5.4.1 item 2; `NT-92-004`; `SR-94-048` |
...
| AB-74-009 | Informational alerts are provisional. They never assert that a limit was exceeded and self-clear when the underlying provisional record resolves (`SD-071-043`). They expose no acknowledgement operation or state, and a direct acknowledgement request is rejected without state change — the stronger rule, from CBD-72 permission 13. | `SD-071-043`; CBD-72 permission 13 |
...
| AB-74-012 | Cooldown and deduplication are system-owned. **At most one** shared event exists per built-in rule, source identity, and material source revision — at most rather than exactly one, because §8.2 cooldown may suppress an otherwise-eligible repeat, which `AE-74-02` records. Retries and duplicate triggers create no second event or instance. | `SD-071-044`; CBD-72 §5.4.1 item 1; CBD-74-AC08 |
...
`CAT-74-06`'s firm classification is **derived, not quoted**: `SD-071-043` names the firm set as settled overspending and late-adjustment overage without listing income variance, while `PD-68-16` fires it only on *confirmed* reconciliation, which meets the `SD-071-043` definition of a settled fact. `PD-68-16` itself says nothing about self-clearing; that property is inferred from the settled-fact classification rather than quoted, which is part of what `OI-74-001` asks the Product Owner to confirm. The derivation is recorded as `OI-74-001` for explicit Product Owner confirmation and is not treated as settled by this draft.
...
| --- | --- | --- | --- |
| MN-74-01 | Membership and consent lifecycle notices — invitation outcome, role or scope change, revocation, removal, ownership transfer | CBD-73 §11.3 `RC-73-12`, §7, and `docs/cbd-73-customer-message-inventory.md` | Mandatory authenticated in-app instance, created independently of membership authority. **Two recipient classes, not one.** The **subject** of the event receives the `IC-73-019` treatment: external copy only to their verified private safety channel, with no fallback, unaffected by any alert preference, quiet hour, digest, or dismissal. **Other recipients**, such as owners notified under `RC-73-12`, receive their notice under their own entitlements and their own ordinary personal delivery preferences (CBD-72 §6.3); they are not routed to a safety channel and are not exempt from their own preferences. External content for **both** classes is capped by §6.1 and §6.2 exactly as alert content is: push and SMS carry the fixed content-free body, and lifecycle email may state only the safe action class, whether action is required, and a deadline (`EM-92-003`; CBD-92 `PA-92-008`). A revocation notice is the highest-risk message the product sends, and no exemption from the content ceiling exists for it. |
...
6. **`RI-93-014` residual, accepted not mitigated.** Retaining Partner informational eligibility means an active Partner can observe a provisional condition about an identified household before the people in it have corrected it. The Product Owner retained this on August 16, 2026 and formally accepted the residual risk on September 2, 2026 as the named risk authority under CBD-94 §3.6 (`OI-74-006`, closed). Acceptance is not mitigation: the observation and surveillance risk is unchanged, and `AB-74-008` limits the harm by removing actor attribution without removing the residual. This rule states it so that no later reader mistakes a closed issue for an absent risk.
...
4. For each recipient with a matching opted-in external channel, delivery is scheduled subject to quiet hours (§8.3) and digest choice (§8.4).
5. **At rendering and again at send**, each as its own check, the delivery boundary rechecks recipient identity, destination ownership and version, opt-in, current eligibility, authorization and lifecycle versions, template version, material revision, and suppression state (`SR-94-048`; `NT-92-004`; `EM-92-006`). `SR-94-048` requires both; a quiet-hour or digest deferment can separate them by hours, and a recipient may lose eligibility in between. Any failed recheck at either point suppresses the attempt, and a message rendered before a loss is discarded rather than sent.
...
2. A recipient who lost eligibility entirely cannot open the instance, and the denial discloses neither the content nor the reason beyond a safe outcome class. `SD-071-047` requires a denied attempt to change no state, show a clear explanation, and be audited; it governs denied **mutations**, where the actor already knows the target exists. A denied **read** here must additionally not confirm the target's existence, so the explanation is the non-revealing kind CBD-72 §5.2 permits: clear about what the reader may do next, silent about what was withheld. Both rules are satisfied; neither is relaxed.
...
5. **A locator leaks nothing on the way in.** Any link an external notice carries locates a generic authenticated entry point and authorizes nothing (`EM-92-004`). It must not place a resource, event, space, or recipient identifier in a path, query string, or fragment; must not pass one to a third party through a referrer header, redirect chain, or client-side analytics; and must not record one in an access log that a wider audience can read than may open the instance itself (`SR-94-047`; `SR-94-049`). A denied or expired locator returns the same safe outcome class as any other denied open, disclosing neither the target nor the reason.
...
4. **Comments** are attributed interaction records on supported readable targets, governed by CBD-72 §5.6. **Who may comment**: Primary Owner, Co-owner, Collaborator, and Accountability Partner. **A Viewer may not comment**, on an alert target or any other, under any profile — CBD-72 permission 11a denies it, §5.1 item 11 excludes it, and §5.6 item 3 states it is not part of MVP. Viewer eligibility for a firm alert under §4.3 rule 2 grants read access to that instance and nothing more. Only the author may edit or remove their own comment; no role may moderate another author's comment.
...
7. **Dismissal is not resolution.** Archiving or dismissing an instance removes it from that person's active view only. It does not resolve the condition, clear another recipient's instance, confirm or reject a reconciliation, or alter the shared event (CBD-72 §5.4.1 item 4; `AB-74-012`). `PD-68-16` governs the `CAT-74-06` trigger and its deduplication, not dismissal, and is not cited here for it.

## 8. Cooldown, deduplication, quiet hours, digest, dismissal, and pause

### 8.1 Deduplication

At most one shared event exists per built-in rule, source identity, and material source revision; §8.2 cooldown may suppress a repeat that would otherwise qualify. A materially changed source revision may create one new linked event; an immaterial change creates none. Delivery retries never create another event or instance (`AB-74-012`; CBD-72 §5.4.1 item 1).
...
A recipient who chooses digest rather than immediate delivery (`CF-74-04`) receives their own external notices batched. The batch is a delivery-timing choice and nothing more.

1. **A digest changes no in-app behavior.** Every eligible in-app instance is still created immediately at §5.3 item 3, whatever the digest choice (`AB-74-002`).
2. **A digest carries no more content than a single notice.** The §6.1 ceiling and the §6.2 prohibitions apply to the batch exactly as they apply to one message. A digest therefore does not state how many notices it covers, which categories they belong to, or when each occurred; a count is itself customer-specific content under the §6.2 residual clause.
3. **Each batched attempt passes its own rendering and send-time recheck** (§5.3 item 5). An instance that became stale, or whose recipient lost eligibility, is dropped from the batch rather than delivered late, and its removal never changes what the rest of the batch says.
4. **A digest is per recipient, per membership, per channel.** Notices arising in different budget spaces are never combined into one batch, even for the same person on the same destination (`AB-74-011`; `PB-74-08`).
5. **Digest cadence is system-owned within the recipient's choice.** The recipient selects digest or immediate; the exact cadence, batch window, and ordering are execution-level (`OI-74-004`) and are not presented as configurable values.
6. **An empty batch is not sent.** If every instance in a window is dropped, no message is delivered, because sending an empty notice would disclose that something occurred and was withdrawn.

### 8.5 Dismissal and pause
...
| CP-74-08 | Alert, settings, and acknowledgement surfaces meet the `SD-071-048` accessibility requirements. `SD-071-048` names notification settings among its surfaces; extending the same bar to alert display and acknowledgement is this document deliberately applying it more widely, not an inherited obligation. The requirements are: programmatic labels and relationships, keyboard operation, managed focus, announced state changes, non-color cues, and accessible recovery. |
...
| DR-74-05 | Verified notification destination | Recipient account; channel type; destination or token with verified ownership; verification and rotation state; suppression state from provider callbacks. Separate from authentication and recovery authority. It is the recipient's personal data: it is retired when they remove it, when verification lapses, or when a provider reports it permanently invalid; a retired or provider-invalidated destination stops receiving delivery immediately and is not retained beyond the audit correlation that proves the cutoff. Retiring a destination never affects that person's sign-in or account recovery (`SR-94-051`; `SR-94-052`). | `DI-91-029`/`DI-91-049`; `SR-94-051`/`SR-94-052` |
...
| --- | --- | --- | --- |
| OI-74-001 | **`CAT-74-06` class confirmation.** `SD-071-043` does not name confirmed actual-income variance in the firm set, while `PD-68-16` gives it settled-fact behavior. | Firm: acknowledgeable, no self-clear. It is never treated as informational, and no Viewer receives it unless the whole fact is readable. | **Closed September 2, 2026.** The Product Owner confirmed the firm classification: a confirmed variance is a fact that has happened, so it is acknowledgeable and must not clear itself. The classification remains derived rather than quoted, and §4.1 says so; the derivation is now approved rather than provisional. |
...
| OI-74-006 | **`RI-93-014` residual disposition.** Partner informational eligibility is retained; an active Partner can observe a provisional condition about an identified household before the people in it have corrected it. | Eligibility per §4.3; `AB-74-008` removes actor attribution. | **Closed September 2, 2026 by acceptance, not by mitigation.** Alexander Wohlford, as Product Owner and the named risk authority under CBD-94 §3.6, accepted this residual risk on that date. The risk is real, is not mitigated by `AB-74-008`, and is not reduced by any rule in this specification; it is accepted with its consequences understood. §4.3 rule 6 states it plainly and continues to, so no reader mistakes acceptance for absence. |
