...
| Late adjustment | A change to a completed budget period's actual totals caused by a transaction that settles after the period's end date. Never changes the period's boundaries, schedule-version reference, or planned allocations. |
...
| Refund | A settled return of funds for a previously completed (settled) expense, recorded as its own event with its own posted date and an optional link to the original expense. Distinct from a reversal (§9.3). |
...
| Receive informational (pending) alerts | Yes | Yes | Yes | No | Only if separately opted in (§6.4) |
...
This table resolves the CBD-69-scoped question of *which actions require elevated permission and who is presumptively eligible*. It does not replace CBD-12's authority over exact role definitions, grant mechanics, or enforcement (see **OD-69-04 resolution** in §7.4).
...
| EC-69-02 | Expense lacks a reliable authorization date | Posted / settlement date | Posted / settlement date | Settled on import | Settled category spending in the period containing the posted date | Budget-date and statement views agree (same date used for both) | Standard settled-activity alert rules apply | "No authorization date was available; we used the posted date." | Missing-authorization-date and fallback provenance recorded |
...
| EC-69-07 | A pending and a posted record plausibly represent the same event but lack a reliable link | Authorization date of the pending candidate, pending manual resolution | Provisional, pending resolution | Duplicate-review | Only one instance is ever counted toward category and period totals while unresolved | An unresolved-match indicator appears on both candidate records | Neither record independently triggers a firm overage alert while unresolved | "We found two possible matches for this charge. Review and confirm." | Automatic-match evaluation, presented candidates, and eventual resolution recorded |
...
| EC-69-13 | Refund for a previously settled expense (§9.3: settled return, distinct from a reversal) | Posted / receipt date of the refund | Posted / receipt date of the refund | Settled | Reduces net spending in the period containing the refund's own posted date; the original expense's period and amount are unchanged | Refund appears as its own linked line item; original expense total is not retroactively altered | No overage-style alert; a refund may resolve an existing overage in its own period | "This refund is dated to when it posted, not to your original purchase." | Refund event, link to original expense, and posted date recorded |
...
* **Split** — an authorized user associates one pending record with more than one posted record, or vice versa, for the rare case where an institution posts a single authorization as multiple settlements (or the reverse). Each resulting record retains its own amount and audit trail; the sum of split records must equal the original amount or the split is blocked pending correction.
...
* **Refund** — a previously *settled* expense is returned, in whole or in part, after the fact. Something was spent and later given back. The refund is its own settled event with its own posted date, optionally linked to the original expense for traceability, and reduces net spending in the period containing the refund's own date — not retroactively in the original expense's period (EC-69-13).
