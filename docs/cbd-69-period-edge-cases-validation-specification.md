...
| Reviewer | Alexander Wohlford — Product Owner review performed August 12, 2026 (traceability record §9). Independent third pass still outstanding (RF-69-06). |
...
| Firm alert | An alert that states a **fact** about settled activity. It supports acknowledgement and does not clear on its own, because the fact it reports remains true until the underlying financial data changes. Firm alerts reach the full eligible role set by default (§4). |
| Informational alert | An alert that states a **possible** outcome arising from activity that is not yet final. It never asserts that a limit has been exceeded, only that one would be under current provisional activity. It clears itself when the underlying provisional record resolves and requires no user action. |
...
| INV-69-23 | An informational alert never asserts that a limit has been exceeded, clears itself when its underlying provisional record resolves, and requires no user action. A firm alert asserts a fact about settled activity, supports acknowledgement, and does not clear on its own. | Keeps a provisional hold from being communicated as a settled fact, and makes the firm/informational labels carry a real user-visible difference rather than only an internal classification. | Read the copy of every alert type and confirm no informational alert asserts an exceeded limit; resolve a provisional record and confirm its informational alert disappears without user action; acknowledge a firm alert and confirm the acknowledgement is recorded and the alert does not self-clear. |
| INV-69-24 | A Viewer's firm-alert eligibility follows the same explicit-provisioning scope as their other financial visibility in §4. A Viewer never receives an informational alert, regardless of what is otherwise provisioned. | Lets a provisioned Viewer receive settled facts about data they can already see without also receiving passive notifications about still-changing activity they hold no permission to act on. | Provision a Viewer for a category, trigger both a firm and an informational alert against it, and confirm only the firm alert is delivered. |
| INV-69-25 | An Accountability Partner receives informational alerts by default, on the same footing as firm alerts. Only a Primary Owner or Co-owner may mute an Accountability Partner's informational alerts, and doing so never changes that Partner's firm-alert eligibility. | Matches default informational delivery to what INV-69-23 now guarantees the alert cannot do, while preserving a household-controlled safeguard against the surveillance and coercion risks CBD-12 identifies for the role, without weakening the accountability signal itself. | Assign an Accountability Partner and confirm an informational alert is delivered with no separate opt-in action; mute informational alerts as the Primary Owner and confirm firm alerts still fire; attempt the mute as the Accountability Partner or a Collaborator and confirm it is denied. |
...
| Receive informational (pending) alerts | Yes | Yes | Yes | No, regardless of provisioning (§12.2) | Yes, by default (§12.2) |
| Receive firm (settled overage / late-adjustment) alerts | Yes | Yes | Yes | Only when explicitly provisioned | Yes, when provisioned |
| Mute informational alerts to an Accountability Partner (§12.2) | Yes | Yes | No | No | No |
...
Every alert in this specification is either **informational** or **firm**, as defined in §3. The distinction is not a severity label: it determines what the alert is permitted to assert, whether it clears itself, whether it supports acknowledgement, and who receives it by default (INV-69-23). An informational alert may show that provisional activity *would* exceed a budget; it may never present that overage as final. A firm alert reports settled activity and therefore states a fact.
...
**Viewer eligibility (INV-69-24).** Every other Viewer visibility row in §4 follows the explicit-provisioning scope pattern: a Viewer sees what has been explicitly shared with them. Alert eligibility follows that pattern for firm alerts — a Viewer provisioned to see a category's settled activity may also be provisioned to receive its firm alerts, since the alert reports a fact about data already visible to them. It does **not** follow that pattern for informational alerts, which report *still-changing* provisional activity that a Viewer, who holds no override or reconciliation permission (§4), cannot act on. An informational alert delivered to a Viewer would be a notification about something they can neither influence nor treat as final; CoBudget therefore never delivers one, regardless of what is otherwise provisioned.

**Accountability Partner eligibility (INV-69-25).** An Accountability Partner receives informational alerts by default, on the same footing as firm alerts. This follows from what an informational alert is now precisely defined to guarantee (INV-69-23): it never asserts that a limit has been exceeded, it clears itself without action, and it requires nothing of the recipient. Given CBD-67's grant of comprehensive read-only visibility to the role, withholding only this gentlest alert category by default was a stricter posture than the rest of the role's access already reflects.

Default delivery is paired with an explicit safeguard: a Primary Owner or Co-owner may mute informational alerts to an Accountability Partner at any time, independently of that Partner's firm-alert eligibility, which is never affected by the mute. The Accountability Partner cannot mute their own informational alerts. This separation exists because CBD-12 identifies surveillance and coercion as live risks for the role; a mute control lets the person being supported quiet a passive feed of someone else's still-changing pending activity without weakening the accountability signal — firm alerts, which report only settled fact — that the relationship exists to provide. Muting and unmuting are permissioned actions and are audited under §13.
...
* The Accountability Partner mute control (INV-69-25) is a new interface commitment introduced by this document; it has no prior specification elsewhere and its delivery-channel and notification mechanics beyond the product-level behavior defined here remain CBD-12 scope.
...
| 0.9.4 | August 12, 2026 | Alexander Wohlford with Claude assistance | Defined **firm** and **informational** alerts, which v0.9.3 and earlier used throughout INV-69-15, the §4 permission matrix, six classification-matrix rows, §12, §14, and §15 without ever defining either term — neither appeared in §3 while twenty-odd less consequential terms did. Both are now defined on the axis the CBD-69 Description points at, assertion strength: an informational alert states a possible outcome of provisional activity and may never present an overage as final, clears itself when its provisional record resolves, and needs no user action; a firm alert states a settled fact, supports acknowledgement, and does not self-clear. Added INV-69-23, a §12 lead paragraph, and the resulting §14 interface, §15 data-model, and §16 test consequences. Two of the four consequences are new product rules rather than clarifications — firm alerts support acknowledgement, informational alerts require no action — and were approved by the Product Owner. Delivery channels, thresholds, cooldowns, and quiet hours remain CBD-12 scope. Invariants 22→23, fixtures 32→33. |
| 0.9.5 | August 12, 2026 | Alexander Wohlford with Claude assistance | Product Owner decision on Accountability Partner and Viewer alert eligibility, the open question carried since v0.9.3. **Viewer alerts split:** firm alerts now follow the same explicit-provisioning scope as every other Viewer visibility row in §4 (INV-69-24); informational alerts remain categorically excluded, since a Viewer holds no permission to act on still-changing pending activity. **Accountability Partner informational alerts default on:** given what INV-69-23 now guarantees an informational alert cannot do, withholding it by default was a stricter posture than the rest of the role's automatic access already reflects (INV-69-25). Paired with a new **mute control**, available only to a Primary Owner or Co-owner, that silences informational delivery to an Accountability Partner without affecting that Partner's firm-alert eligibility — a safeguard against the surveillance and coercion risks CBD-12 identifies for the role. The mute control is a new interface, data-model, and audit commitment (§13, §14, §15) with no prior specification elsewhere in this document. Invariants 23→25. |

**Approval status:** Not yet approved. A Product Owner review of this package was performed on August 12, 2026 and produced seven findings (RF-69-08 through RF-69-14), all resolved in versions 0.9.3 through 0.9.5; no product-definition question raised in that review remains open. Approval remains blocked on two gates recorded in the traceability record §7: **RF-69-01**, the CBD-68 reconciliation that holds AC-13 at Provisional, and **RF-69-06**, an independent review pass by a party independent of both authorship and approval. See the traceability record §7 and §9 for the current status of each.
