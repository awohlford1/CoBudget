...
| OQ-108-063 | **Does CoBudget's `TD-103-004` scheduler job carry a body or sensitive headers, and is a target URL itself CoBudget data?** `EV-102-279` establishes a job stores a body only when one is specified, and that headers are stored. | **A design choice and a `DI-91-*` judgment, not a retrieval.** The first decides whether Cloud Scheduler holds anything; the second decides whether what remains counts. Headers carrying credential material would be `HG-102-014` territory, not this gate's |
