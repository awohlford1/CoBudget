...
| CBD-103 §3.3 | *"exception rule `EX-102-003` cannot help here, because an exception converts a `FAIL`, and these gates are `UNPROVEN`. **`UNPROVEN` has one remedy " + D + " better evidence**."* |
...
| OQ-108-070 | **Should CBD-15's scope be amended a second time to authorise provider contact for `D4` documentary gates, as it was amended on August 20 for the ten `OBS` gates?** Route A authorised accounts *"for the sole purpose of running the ten pass tests"*; `HG-102-013` is typed `DOC` and no trial account can observe a contract. | **The exact parallel of route A, one class of evidence over.** C1's `UNPROVEN` has a remedy in principle and no authorised route in practice. **This is the live blocker on C1**, and C2's and C3's `D4` items sit behind the same gap |
...
| OI-108-055 | **Two merged documents carried unrendered template placeholders.** The exception records at disposition register §5 and `EV-102-243`/`EV-102-244` in this document shipped with literal `{D}` and `{S}` tokens, and one line leaked Python source into a quotation of AWS text. Repaired at tranche 44. | **All ten checks passed on every one of those commits.** The audits verify identifiers, tallies, versions and baselines; **nothing verifies that the prose renders.** Recorded rather than fixed in the scripts, because the generation-time bug — a `.format()` omitted on one insertion — is not something a document audit can see. |
