...
| OQ-108-075 | **Does Privileged Identity Management satisfy `HG-102-005` at all?** The pass test asks the access model to support *"expiry and revocation conditions bound to a named change or incident record"*. `EV-102-297` records PIM supplying start/end dates, approval and **free-text justification**, and stating no binding to a named record. | **Prior to `OQ-108-009`, which assumes it does.** §4.52's check run early: before asking whether an alternative to PIM clears the gate, establish that PIM does. **C3's identified floor of $61.41 is conditional on the answer** |
