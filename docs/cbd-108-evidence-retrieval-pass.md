...
| OQ-108-073 | **Does SQS's fourteen-day maximum reach its dead-letter queues?** `EV-102-250` records that SQS cannot hold a message beyond fourteen days, and an SQS dead-letter queue is an ordinary SQS queue — but that record addressed SQS generally and not DLQs. | **The `OQ-108-066` discipline applied before the claim rather than after.** If the ceiling reaches SQS DLQs it is a **genuine C2/C3 differentiator**; if it does not, the unbounded dead-letter queue is a shared property of brokers and §4.38's reading extends again |
