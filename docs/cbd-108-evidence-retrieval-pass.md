...
| OQ-108-072 | **Can Azure Service Bus entity expiration be given a provider-enforced ceiling, or is a broker substitution required?** `EV-102-259` records the default as *"the largest possible value for a signed 64-bit integer"* on standard and premium with no stated ceiling. | **The C3 counterpart of `OQ-108-040`**, which asked the same of C2's log sink and found the substitute worse. **This is what would actually move C3**, where the edge swap would not — and §4.38's caution applies: check the substitute before recommending it |
