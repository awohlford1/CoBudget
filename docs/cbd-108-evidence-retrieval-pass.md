...
* **It does not clear a gate.** `CR3` — cost never overrides a gate. Every selection
  remains at `ELIGIBLE-PENDING-EVIDENCE` and the route-A observations are still
  owed — **twenty-seven of them**, scoped one tranche later at §4.83. An
  affordable set is not an evidenced one.
...
| **2.** C1 hosting chosen to reduce vendor count | **No.** C1 was **forced**, not chosen for consolidation: C2 and C3 are `INELIGIBLE` on `HG-102-013` and C1 was the only candidate standing. The caution survives as a description rule — the set must never be described as consolidating, because choosing C1 **raised** the count from one ecosystem — but no selection rests on the reasoning the finding rejects. **Corrected at tranche 76: the realised set is three vendors, not four** — §4.87 |
...
### 4.87 The price-side narrative outlived the price side — added at v0.80

**Tranche 76, September 5, 2026.** Tranche 71 completed the cost model. **Six
places across three documents went on saying it could not be completed**, and two
of them contradicted newer text on their own page. Found by reading the package
as an approver would, in submission order, rather than by an audit.

#### What was stale

| Where | Said | Since |
| --- | --- | --- |
| Cost model status row | *"**It cannot be produced as a figure**… an empty price side"* | Tranche 71, four days |
| Cost model §4 title and closing | *"The price side, which is empty"*; *"no honest way to present that as a low/base/high range"* | Tranche 71 |
| Cost model §4 rows I and F | Identity *"the only category with figures"*; F *"No rate is published"* | Tranches 68 and 71 |
| Cost model §2 | *"`OI-102-017` — no budget ceiling exists"* | Tranche 66, the ceiling decision |
| Traceability AC4 | *"`OI-108-011`… remains open"* | Contradicted by its own deliverables table |
| **Traceability AC7 residual 3** | *"**No cost totals or thresholds**"* | Tranche 71 |
| Disposition §6 and `OI-108-002` | *"thresholds cannot be set… `UNKNOWN` in five of six"* | Contradicted by §6 four lines above it |

**AC7's residual list is the one that mattered.** It is the list the Product
Owner signs, and it asked for acceptance of a risk that no longer existed. A
residual-risk acceptance is worth exactly the accuracy of its list: one item
that is false does not merely waste a line, it puts the other four in doubt.

#### Why the audit did not catch it

`scripts/audit-cbd-108.py` passed 117 checks over every one of these commits.
Tranche 71 inverted the cost-model guard so it requires `$62.56` and forbids
*"no combined total is produced"* — **the exact sentence that was stale then**.
The six that survived use different words for the same claim.

**This is the third instance of one failure mode**, and the first where the
remedy is not another string. `OI-108-079` recorded it at tranche 65 for the
disposition register's header, and tranche 75 ended one instance by removing the
Companions version rather than resetting it. **A guard pinned to a sentence
catches that sentence.** What decays is the claim, which each document phrases
its own way.

#### The guard now matches the claim, not the sentence

`scripts/audit-cbd-108.py` gains a **family** of seven patterns for the
price-side claim — *the price side is empty*, *the total cannot be produced*, *no
combined total exists*, *no cost totals*, *thresholds cannot be set*, *no budget
ceiling exists*, and *prices unknown in N of six categories*. The audit goes from
117 checks to 152.

**Two exemptions, and both are load-bearing.** Struck and quoted text is exempt,
because the table above quotes all six stale claims verbatim in order to record
them — a guard that fails on its own correction is the §4.77 defect, *"a guard
that requires a false statement is worse than no guard"*, in its mirror form.
And the guard skips this document and any `Revision record` section, because a
dated ledger entry describing what was true at v0.30 is **the record**, not a
defect; the exemption is anchored to the heading so prose cannot drift into it.

**Proven by deliberate violation**, per the standing rule that a guard is
finished only once it has failed a build. Each of the seven claims was
reintroduced one at a time — **into a document other than the one it originally
lived in**, so the guard is shown to be about the claim rather than a remembered
location — and each failed the audit. Both exemption controls were checked in the
same run and correctly ignored.

**What the guard does not do is fix the cause.** A status row and a
residual-risk list are **derived** views, and the way to stop them decaying is to
stop restating a conclusion the package computes elsewhere. `OI-108-082` records
that as the structural remedy and does not take it.

#### The vendor count, corrected

§4.86 said at tranche 75 that *"the set is four vendors"*. **It is three** —
Google Cloud for hosting and database, AWS for identity, email and SMS, Plaid for
financial connectivity — which the disposition register §4 states twice and
§4.73 states as *"two clouds"*.

The four came from the coherence review, and it was right when written:
§3 projects that *"a C1 set spans four vendors plus an aggregator"*, and §8's
finding 2 says C1 *"raises the count from one ecosystem to four vendors"*. **The
projection was overtaken by the selections themselves** — identity, email and SMS
all went to AWS, which the review had no reason to assume. **Seventeenth entry in
the correction ledger**, and its shape is new: not a misread document and not a
missed record, but a **superseded projection quoted as a current fact**. The
source was accurate; it had simply been outrun by decisions taken after it.

**It does not move finding 2's disposition.** C1 was forced rather than chosen,
so no selection rests on the consolidation reasoning the finding rejects, and the
description rule survives at three vendors exactly as it did at four: choosing C1
...
### 4.88 The package is approved, and what that does not do — added at v1.0

**Tranche 77, September 5, 2026.** The Product Owner approved CBD-108 and its
five residual risks. **AC7 is met, all seven acceptance criteria are met, and the
six documents move from Draft v0.80 to Approved v1.0.**

#### What was approved

| | |
| --- | --- |
| **The provider set** | C1 Google Cloud (hosting), C1 Cloud SQL (database), C2 Cognito (identity), C2 SES (email), C6 Plaid (financial connectivity), C2 AWS End User Messaging (SMS), Web Push deferred. **Three vendors** |
| **The cost model** | `$45.57` / `$62.56` / `$164.21` monthly; `$546.84` / `$750.72` / `$1,970.52` annually. Every scenario inside the `$175` warning and the `$250` ceiling |
| **The residuals** | Five, accepted rather than retired — traceability AC7 |

#### What approval does not do

**It converts no verdict.** All six selections stay at
`ELIGIBLE-PENDING-EVIDENCE`. The twenty-seven route-A observations stay owed.
Evidence register §3.3.1 permits selection at that verdict **for the Private MVP
phase only**, so the approval is phase-scoped and the set is not cleared beyond
it.

**It clears no gate.** `CR3` holds — cost never overrides a gate — and an
approval is not evidence. The coherence review still certifies nothing, four
cross-cutting `X` gates remain `UNPROVEN` on every selected candidate, and
`HG-102-013` remains the gate that could fail all of them at once.

**The risk CBD-103 §3.3 stated when it offered route B is now accepted twice
over**: once on September 2 when the selections were made, and once today when
the package carrying them was approved. *"A gate that fails on first observation
invalidates the selection after integration work has started."* That sentence is
the substance of residual 1, and approving the package does not make it less
true.

#### The guard inverts, for the third time in this package

`scripts/audit-cbd-108.py` required every document to carry the words *"Draft —
not approved"*. **That requirement became a requirement for a false statement the
moment the Product Owner approved**, which is the §4.77 defect exactly — *"a
guard that requires a false statement is worse than no guard"*. It now requires
the approval and forbids the draft string, and was shown to fail against the old
text before the new text was written.

**Third inversion of this shape.** Tranche 65 inverted the disposition register's
*"selects no provider"* guard; tranche 71 inverted the cost model's *"cannot be
produced"* guard; this one inverts the status-row guard. Each was a guard pinned
to a true sentence that a decision later falsified. `OI-108-085` records the
pattern, which is not the same as `OI-108-082`'s: **that one is prose drifting
from an artifact; this one is a guard drifting from a decision.**

#### An eighth price-side survivor, found while promoting the register

The disposition register's status row said the package had *"no combined cost
