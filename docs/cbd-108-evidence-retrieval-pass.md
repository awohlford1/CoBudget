...
| OQ-108-069 | **What instrument lets a candidate proceed on an `UNPROVEN` gate?** `EX-102` exceptions presuppose a `FAIL` to proceed despite, and `EX-102-003` bars converting one to `PASS`. C1 is `UNPROVEN` on `HG-102-013`, so `EXC-108-001` is withdrawn at §4.56 with nothing recorded in its place. | **Product Owner's, and this package does not invent an instrument.** The alternative is the `D4` retrieval — one statement from Google bounding destruction after routine expiry, which would move C1 to `PASS` outright rather than by exception |
