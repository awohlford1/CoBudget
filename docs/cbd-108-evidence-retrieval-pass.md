...
| OQ-108-065 | **What does Google state about Cloud Logging's routine retention-expiry timing?** `EV-102-262` records the buckets page silent on it, giving only a 7-day grace on **shortened** retention. | **The only thing between C1 and a `PASS` on `HG-102-013`.** A single retrieval on a page other than the one already read. Google pricing pages have failed eight times, but documentation pages have not |
