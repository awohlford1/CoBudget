...
| OQ-108-027 | ~~Should the `HG-102-013` catalog row be amended so its gate statement and pass test agree?~~ **Specified at §4.35, on two independent grounds.** First the wording: the §4.24 ruling settled the reading, not the text. Second and more serious: **no candidate's contract describes a provider-held backup at all**, so the row asks about an object none of the three treats as a distinct artifact. | **Still CBD-102's to decide, and now fully specified.** Read literally the gate fails every candidate in every category at once, which answers the upstream `OQ-103-024` in the way it feared. Raises `OQ-108-039` |
