...
| OQ-108-049 | Does `HG-102-024`'s *"non-exportable keys **where supported**"* require **HSM protection**, and therefore Key Vault **Premium** for C3? `EV-102-267` establishes HSM keys are *"N/A"* on Standard. The *"where supported"* qualifier may make it optional. | **Decides whether C3 must use Premium or Standard.** *(Originally framed as deciding `CT-102-001`; `EV-102-276` shows Premium has no plan fee, so the cost is `CT-102-007` at $1 per key per month — or $2,336.00/month if a Managed HSM pool is required, `OQ-108-056`.)* Also needs the equivalent question put to C1 and C2, which §4.41 flags as the unchecked half of the row |
...
| OQ-108-056 | **Which Azure HSM product does `HG-102-024` require**, and how many keys and surviving versions would the C3 composition hold? Key Vault Premium HSM keys are **$1 per key per month**; a Managed HSM pool is **$3.20/hour, $2,336.00 per month** (`EV-102-276`). | **A 2,336× spread on one drafting question.** C3's identified floor is either about $50 or about $2,385, and **the cost model cannot be completed for C3 without this answer**. The key count matters too: each active version counts separately |
