...
| OQ-108-009 | Can `HG-102-005` be satisfied for C3 **without** Privileged Identity Management? ~~and what does Entra ID P2 cost per administrator seat?~~ **The price half was answered at tranche 31: Entra ID P2 at $10.00 per user per month.** | **Narrowed to the gate half.** `EV-102-229` establishes what PIM requires, not that PIM is required — and C3's identified floor of $61.41 is conditional on the answer |
...
| OQ-108-021 | Does CoBudget hold data in AWS that is **not Personal Data**? **Narrowed at §4.60 to one candidate**: `DI-91-041`, *"allowlisted operational logs and aggregate health metrics"* at **S1**, whose aggregate half is the only material in the inventory that plausibly falls outside the definition. Every other class is subject-linked. | **Still a Product Owner judgment**, as the row always said, and §4.51's precedent puts `DI-91-*` classification with the Product Owner. **The asymmetry is worth noting**: §8.2's carve-out reaches *"Your Content"*, which is wider than the Personal Data the commitment covers |
...
| OQ-108-074 | **What constitutes *"a full re-evaluation"* under `EX-102-006`?** **Worked at §4.66 as far as the corpus determines it.** No section defines it, but four constraints follow from text, and the corpus's **three other** uses of *re-evaluation* — demand-model headroom and two multi-region triggers — all denote **the event that ends the Private MVP phase**. | **Narrowed to two readings**, provider-scope and selection-scope. **A is recommended and not adopted** — §4.65 reversed a confident reading of text one tranche ago. **Definition belongs in CBD-102.** On either reading the exception route is not a fast path, confirming §4.59 |
