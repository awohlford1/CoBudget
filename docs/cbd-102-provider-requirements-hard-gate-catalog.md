...
**Filling the role — Product Owner disposition, August 22, 2026.** `OI-102-022`
recorded that the role is defined but unfilled, with no provider access
provisioned. It is **deferred to the build phase**, because the role cannot be
provisioned meaningfully until there is infrastructure to provision access on,
and CBD-15 provisions nothing. It is deferred against a hard precondition rather
than a date: **no production restore, no key recovery, and no exceptional access
may occur before the role is filled**, and `ME-94-013`'s recovery rehearsal
cannot be signed off without it. The authorized observation pass is unaffected,
because it runs on synthetic data in non-production evaluation accounts where no
customer content exists to separate.
...
| HG-102-006 | X | Backup data access, key recovery, restore execution, and approval are separable so that no single operator can combine customer data and keys, or return a restore to service unilaterally. Key custody and restore approval must additionally be holdable **without** the ability to read customer data. | Distinct permissions exist for backup read, key use/recovery, and restore execution, and can be held by different principals. **Restore approval is satisfied where the provider can require approval by a distinct principal before the restoring capability can be exercised — including approval bound to the *activation* of the role that performs the restore, rather than to the restore call itself.** A provider where one role implies all four fails. A provider also fails where key custody or restore approval implies customer-data read, since that collapses the §2.5.1 second-principal boundary regardless of role naming. **Firm — see §2.5. Amended August 22, 2026 — see §12.1.** | `OP-92-006`; `SR-94-069`; `SR-94-081` | Vendor |
...
| HG-102-013 | X | Provider-held copies of CoBudget data have a stated, bounded lifetime and a stated location, and neither may be assumed from CoBudget's own backup policy. | A contract or written evidence states — for the provider as a whole, or for each component of **the evaluated composition** — the maximum period, after a deletion instruction **or after the expiry of a configured retention period**, within which the provider will have destroyed every copy of CoBudget data it holds, and the region or geography those copies occupy. A customer-configured period satisfies this **only** where the provider also states a ceiling the customer cannot raise; **the gate tests whether a ceiling exists, not whether it is proportionate**. A stated bound with an unbounded tail fails. Silence fails. | CBD-91 §5.1 provider row | Vendor |
...
| HG-102-074 | N | The provider discloses carrier and platform retention for delivered messages, and CoBudget is not required to rely on platform preview-hiding as a confidentiality control. | Written, dated retention evidence is obtained for the **platform**, together with a written statement of what the provider does and does not control downstream. **The carrier segment is satisfied by disclosure rather than by control**: a provider that states plainly which parts of delivery lie outside its control satisfies this half, and a provider that is silent about the carrier segment does not. `NT-92-006` applies the generic-body rule *even when a platform claims previews are hidden*, so a provider whose privacy story depends on preview suppression fails. **Amended August 22, 2026 — see §12.1.** | `NT-92-006`; `DI-91-049`; `EG-91-024` | Vendor |
...
| OI-102-022 | The §2.5.1 second principal is defined as a role but not yet filled. Nobody currently holds key-recovery custody or restore approval, and their provider access is not provisioned. | **Open — operational, deferred by Product Owner decision on August 16, 2026.** Does not block CBD-103–107 or CBD-130, which only ask whether a provider *can support* the separation. It does block any CoBudget recovery claim, and `HG-102-006` cannot be satisfied in practice until the role is filled, however capable the provider. Needs a name, a provisioning step, and an availability expectation. |

### 12.1 Amendment record

| Date | Change |
| --- | --- |
| August 30, 2026 | **`HG-102-013`'s pass test amended again by Product Owner decision**, settling `OQ-108-041`, `OQ-108-042` and `OQ-108-044`. All three were raised by CBD-108's retrieval pass **about wording that pass had itself proposed**, and all three were exercised against real evidence before being put. **The gate statement is unchanged and nothing it protects is relaxed.** *(1) Proportionality is not tested.* `EV-102-258` records Azure Log Analytics capping total retention at twelve years, which is a ceiling the customer cannot raise and therefore passed the v1.2 text as written. The amendment says so explicitly rather than leaving a reader to infer it: the gate tests **whether** a ceiling exists, not whether it is proportionate. §2.1 requires a gate to be *"a binary provider property"*, and a proportionality threshold is not binary; whether twelve years is acceptable for a given data class is a Product Owner judgment against `DI-91-*`, which is where it now sits. *(2) The test is scoped to the evaluated composition.* v1.2 said *"each evaluated component"*, which read literally reaches every service a provider sells. CBD-108 §4.38 established that **all three hyperscalers disclaim a bound on object-storage lifecycle deletion** — Google most explicitly, *"Your applications shouldn't rely on lifecycle actions occurring within a certain amount of time"* — so the unscoped text would have failed every candidate on a service **no evaluated composition contains**, recreating exactly the correlated failure v1.2 removed. Scoping to the composition keeps the clause's bite where the compositions actually are, and CBD-108 `OI-108-034` records that introducing object storage into any composition re-opens it. *(3) The test reaches routine expiry.* v1.2 measured from *"a deletion instruction"*, and CBD-108 §4.38 found Cloud Logging silent on ordinary retention expiry while Google's CDPA §6.2 speaks only to deletion *"at the end of the Term"*. Data reaching the end of a configured retention period is the same lifetime question as data deleted on instruction, so the test now covers both. **This is the one change with an immediate cost, and it is recorded rather than discovered later: it moves C1's lifetime element from met to not met**, because `EV-102-262` shows Cloud Logging states no routine-expiry timing at all. That is a real finding about a real gap rather than an artefact of drafting, which is why the change was made in this direction. **Every candidate already measured against v1.2 must be re-measured against v1.3**, and since no evaluation has yet re-measured against v1.2, the two obligations collapse into one pass. |
| August 29, 2026 | **`HG-102-013` gate statement and pass test amended by Product Owner decision**, resolving `OQ-108-027` and answering `OQ-103-024`. The gate had been shown unsatisfiable by any candidate, on the same grounds as the August 22 pair. CBD-108's retrieval pass read all three data processing addenda and **none describes a provider-held backup**: Google's CDPA reaches *"existing copies"* but `EV-102-168` records that it *"does not use the word backup or separately describe provider-held backup copies"*; AWS **assigns** backup and archiving to the customer in both the DPA (§5.2, §8) and the Customer Agreement (§2.3); Azure's per-service statements are customer-configured retention of primary data. The previous test asked for *"backup retention, region, and expiry"*, which presumes a backup is a named artifact with a published lifetime — a managed-database framing that no hyperscaler uses. Read literally it made this cross-category `X` gate fail every candidate in every category at once, eliminating the whole shortlist by definition rather than by evidence. **Two defects are fixed together.** The gate statement said *"stated contractually"* while its pass test admitted *"a contract or written evidence"*; the August 29 ruling that the pass test governs settled the reading but not the text, and the amended gate no longer says *"contractually"*. And the test now asks what the gate protects — that copies the provider holds have a bounded lifetime and a known location — rather than asking for an artifact by a name providers do not use. **The gate is not relaxed in what it protects.** It still fails a provider that states no bound; it now also fails one whose bound is purely customer-configured with no provider-enforced ceiling, and one whose stated bound has an unbounded tail, neither of which the previous text caught. On the evidence in hand it continues to fail all three candidates, but **for specific evidenced reasons rather than a definitional one** — C1 on the region element for Cloud Scheduler, C2 on CloudWatch Logs deleting *"up to 72 hours"* after the configured period *"but in rare situations might take longer"*, C3 on components carrying no provider ceiling. **Every candidate already measured against the previous text must be re-measured against this one** — CBD-103, 104, 105, 106, 107 and 130 record that at their own revision records, and CBD-108's disposition register and coherence review restate the position. |
| August 22, 2026 | **`HG-102-006` and `HG-102-074` pass tests amended by Product Owner decision**, resolving `OI-103-020` and `OI-130-010`. Both gates had been shown unsatisfiable by any provider in their markets: no cloud IAM system models "a second person must approve this specific restore", and no messaging provider controls carrier retention. Leaving them unmeetable would have made two gates dead text that no selection could ever satisfy, which weakens the catalog everywhere else. The amendment was chosen over case-by-case reinterpretation because an amendment is visible, dated and reviewable, and because §2.5 forbids widening a firm gate by drafting. Neither gate is relaxed in what it protects: `HG-102-006` still requires four separable duties and still fails a provider where one role implies all four, and `HG-102-074` still fails a provider that is silent about downstream retention or that rests its privacy story on preview suppression. **Every candidate already measured against the previous text must be re-measured against this one** — the evaluations record that at their own revision records. |
