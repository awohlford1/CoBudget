...
| HG-102-013 | X | Provider-held copies of CoBudget data have a stated, bounded lifetime and a stated location, and neither may be assumed from CoBudget's own backup policy. | A contract or written evidence states — for the provider as a whole, or for each evaluated component — the maximum period after a deletion instruction within which the provider will have destroyed every copy of CoBudget data it holds, and the region or geography those copies occupy. A customer-configured period satisfies this **only** where the provider also states a ceiling the customer cannot raise. A stated bound with an unbounded tail fails. Silence fails. | CBD-91 §5.1 provider row | Vendor |
