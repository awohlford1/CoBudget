...
| Field | Value |
| --- | --- |
| Status | **Draft v0.5 — Product Owner review and approval required** |
...
| 11 | CBD-92 `NT-92-001` fixes the literal push and SMS body as "You have a new CoBudget update. Open CoBudget to review." `EM-92-001` and `EM-92-002` name CoBudget the same way in routine and invitation email. These are customer-readable strings in an approved document, and `RT-75-01` requires MoneyPact. CBD-73 and CBD-74 were amended for exactly this on September 2. | **Open.** Not corrected here. CBD-92 is a frozen authority source pinned by `audit-cbd-73.py`, `audit-cbd-74.py`, and `audit-cbd-95.py`, so amending it is its own change with its own re-pin, and this one was already scoped to the CBD-94 §8 rows. Recorded so the naming rule is not quietly incomplete: the transport contracts still say the old name, and any implementation reading them would ship it. |
