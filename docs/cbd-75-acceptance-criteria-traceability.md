...
| Field | Value |
| --- | --- |
| Status | **Draft v0.3 — Product Owner review and approval required** |
...
| 7 | Legacy guardian vocabulary survived in `docs/product-plan.md` and `docs/architecture.md`. The product plan additionally framed the Accountability Partner naming decision as still open, and described the role's access as scoped and revocable by the subject, which CBD-94 records as materially misleading. | **Prose corrected September 3, 2026**, satisfying `SR-94-145` and the two CBD-94 §8 rows for these files. The remaining role-sense uses of the word are gone; the three that survive are the legal sense in the scope exclusion and the deferred-capability list, plus the sentence retiring the term. `OI-75-004` stays open for the four records that still describe the corrected condition in the present tense, because amending CBD-91 or CBD-94 invalidates pinned blob hashes in four places each. |
| 9 | The product plan says a Collaborator cannot manage financial connections. CBD-72 §2.2 permits a Collaborator to authorize and manage the connections they personally authorized, so the sentence is wrong in the same paragraph that finding 7 corrected. | **Deliberately not corrected.** This is CBD-91 `CR-91-002`, a separate recorded contradiction whose disposition states that the product-plan correction "requires separate consent/change". The bullet was left byte-for-byte so the diff shows it untouched. Correcting it is a decision for whoever takes `CR-91-002`, not a side effect of a vocabulary fix. |
