...
| --- | --- | --- |
| `docs/architecture.md` | ~~Add `CL/PA/NT/EM/OP/AN/RL-92-*`, typed authorization/service-purpose contract, tenant/cache/index keys, queue schemas, audit/telemetry boundaries, deletion/recovery ledger, and provider-evidence gates~~ **Done September 3, 2026** as far as a planning note can be: the fifty contracts are carried as binding constraints and all twelve `RF-92-*` decisions are recorded as open gates naming what each blocks. The designs themselves stay unresolved under `RF-92-*`, which is their correct state, not an omission here | CBD-92 normative contracts and `RF-92-001–012` |
| `docs/architecture.md` | ~~Mark Plaid and every named technology/provider as a hypothesis until CBD-15 evidence and selection~~ **Done September 3, 2026**, with one deliberate refinement: the stack is split into what the repository actually implements and what remains a hypothesis behind a named selection gate. Marking Next.js or NestJS unselected would be false, since both are running | CBD-92 provider-independent limit |
...
| `docs/product-plan.md` | ~~Remove optional rollover from MVP or explicitly point to deferred `FF-003`~~ **Done September 3, 2026**; the MVP list points at `FF-003` and states the no-carry-forward rule | Approved cadence scope and future-feature register |
| `docs/product-plan.md` | ~~Reconcile notification scope: mandatory in-app plus opt-in supported email/push/SMS versus the plan's in-app/email MVP and push-in-Product-depth statements~~ **Done September 3, 2026.** MVP now reads mandatory in-app plus opt-in email, Web Push, and SMS. Product depth adds native clients and their native push transports, because CBD-130 `PN-130-003` already places Web Push in Private MVP | CBD-12/CBD-92 channel contracts |
| `docs/product-plan.md` | ~~State product analytics is disabled for Private MVP; early measures may use only the approved coarse, non-drillable aggregates until a new analytics decision~~ **Done September 3, 2026**; the success-measures section carries the `AN-92-001/002/005/007` boundary and states that silence is not approval | `AN-92-*` |
