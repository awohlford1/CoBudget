...
| Field | Value |
| --- | --- |
| Status | **Approved** — Product Owner approved v1.0 on August 18, 2026. The structure a CBD-103–107 and CBD-130 cost comparison must be recorded in. It contains no prices; providers are not selected here. **v1.1 adds `CT-102-022`, a line for a third-party licence a gate requires, by Product Owner decision of September 2, 2026. It closes a `CR4` breach occurring through the template itself — CBD-108 `OQ-108-055`.** **v1.2 re-decides `OI-102-017`, which the August 16, 2026 decision left open to revisit: a Private-MVP budget ceiling of `USD 250.00` per month is set, by Product Owner decision of September 4, 2026, so CBD-108's warning and stop thresholds can be stated. No line item, `CR` rule, billable unit, or price changes.** |
...
| OI-102-017 | **Decided August 16, 2026: no budget ceiling. Reopened and re-decided September 4, 2026: the Private-MVP ceiling is `USD 250.00` per month, `USD 3,000.00` per year.** The warning threshold is 70% of the ceiling, `USD 175.00` per month; the stop threshold is the ceiling itself. Cost still cannot exclude a provider on its own — the thresholds govern review and escalation, not eligibility, because `CR0` ties tier to gate clearance and no automatic tier downgrade is permitted. | Closed as a decision, with a consequence worth stating plainly: CBD-103–107 can produce a fully evidenced, gate-clearing recommendation that turns out to be unaffordable, and there will be no documented basis for having ruled it out earlier. `CT-102-017` and `CT-102-019` are the figures to watch for that, and a ceiling can be introduced later if one becomes obvious. |
