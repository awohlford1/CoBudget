...
| Field | Value |
| --- | --- |
| Status | **Approved v1.0 — mechanical audit passed; Product Owner approved August 16, 2026** |
...
```text
CBD-95 AUDIT PASS: 3930 checks, 0 failures, 0 warnings; 18 upstream families, 11 frozen blobs, 36 RC rows, 30 FU rows, 8 RV findings, 19 RI decisions, 14 required areas, 6 CBD-95 ACs, 10 CBD-14 ACs
```
...
| RV-95-005 | Low version traceability | CBD-73/74/75 descriptions cite CBD-71 v1.0 although their inherited alert behavior depends on approved v1.1; CBD-93 also has stale publication wording. | **Applied August 16, 2026 for the Jira surfaces.** CBD-73, CBD-74, CBD-75, and CBD-76 now cite CBD-71 v1.1; CBD-76 was added to the set by `RV-95-008`. Routed to `FU-95-005`; no behavior changed. The CBD-93 repository header still describes an older published copy and remains open. |
| RV-95-006 | Critical to the CBD-76 dependency control | Blocks links in the early id batch are stored inverse to the documented dependency order. Link `10009` recorded CBD-76 as blocking CBD-95, the reverse of this package's gate. Links created later, ids `10034` and above, are correct. The batch extent is **not established**: first reported as nine, later found to include `10014`, `10015`, and `10016`, so at least twelve. | **Partly applied August 16, 2026.** The five links touching CBD-76 were deleted and replaced in the correct direction: CBD-95 blocks CBD-76, CBD-75 blocks CBD-76, and CBD-76 blocks CBD-77/78/79. Direction was confirmed from both endpoints against the known-correct CBD-94 → CBD-95 link. Jira now enforces CBD-76's downstream position again. At least seven inverted links outside that cluster remain live: `10000`, `10001`, `10002`, `10013`, `10014`, `10015`, and `10016`. The initial sweep queried only issues already in CBD-95's scope and so under-reported the batch; a complete project-wide enumeration is still outstanding and the count may rise. Routed to `FU-95-030`. |
...
| --- | --- | --- |
| CBD-95 document package | **Approved v1.0** | All required artifacts exist, the mechanical audit passes, and the Product Owner approved the exact v1.0 versions on August 16, 2026 with the recorded blockers acknowledged. |
...
| --- | --- | --- |
| Exact repository baseline | Merge commit and the five CBD-95 document blobs plus audit-script blob | The v1.0 approval names the five document versions and the audit script as they stand on `audit/cbd-95`. **These blobs are not yet immutable:** the branch is uncommitted at the time of approval, so the approval binds to the v1.0 content rather than to a merge SHA. The merge SHA must be recorded here once the branch lands, and any content change before merge invalidates this approval under §12 |
...
| Substantive review | `RV-95-001–008` read with affected criteria, current controlling rules, and blocker effects | Complete draft; Product Owner acknowledgment pending. `RV-95-006` must be acknowledged as an active gap in the Jira-enforced CBD-76 dependency, not merely a stale field |
...
- [ ] A complete project-wide enumeration of inverted Blocks links is performed; the batch is known to include at least `10000`, `10001`, `10002`, `10013`, `10014`, `10015`, and `10016`.
- [ ] The intended dependency order for CBD-77 through CBD-81 is established from an authoritative source before those links are rebuilt.
- [ ] The remaining inverted links are corrected.
...
- [ ] The post-merge correction revision is reviewed and merged.
