...
The current candidate was executed on August 18, 2026 with Python 3.12.10
resolved from `PATH`. The portable command is `python scripts/audit-cbd-95.py`.
Exit code was 0.

```text
CBD-95 AUDIT PASS: 4097 checks, 0 failures, 0 warnings; 18 upstream families, 11 frozen blobs, 36 RC rows, 30 FU rows, 8 RV findings, 19 RI decisions, 14 required areas, 6 CBD-95 ACs, 10 CBD-14 ACs
```
...
| Publication | Post-merge Confluence page IDs/versions and parity readback | **Completed for the preceding version set.** All four CBD-95 pages are registered and published: manifest `9797633`, reconciliation matrix `9830401`, follow-up register `9863169`, and this record `9895937`. A readback on August 18, 2026 confirmed that all twelve CBD-91 through CBD-95 pages carry the same document version as their repository source. This revision supersedes those published versions, so `FU-95-029` remains open until it is merged and re-synchronized. |
...
- [ ] Authorized Jira evidence, comments, and status transitions are completed.
