...
| Field | Value |
| --- | --- |
| Status | **Draft v0.2 — audit-hardened; Product Owner approval blocked by `OI-73-001` and the `OI-73-008` Failed-semantics decision** |
...
| Independent review | `docs/cbd-73-exhaustive-review-findings.md` (`RV-73-001`–`RV-73-027`) |
...
2. every `RV-73-001`–`RV-73-027` finding has the evidence-backed disposition in
...
| `docs/cbd-72-collaboration-permission-model.md` | Document version **0.1.53**; front-matter status says approved August 18, 2026 | `21025baf7f50795f318d0fc1decbb5dbef5ae439` | Roles, permissions 24–29, one-role/one-Primary invariants, protected actions, transfer, revocation, ownership exits, attribution and audit schema |
| `docs/cbd-91-private-mvp-data-inventory.md` | Document version **1.0.1**, approved | `1e1134415915238752440da4b2e4acaa293d20e6` | Existing data-class, audience, lifecycle, and prohibited-disclosure boundaries; source alignment and new/split classes remain `OI-73-003` |
| `docs/cbd-94-risk-mitigation-requirement-register.md` | Document version **1.0**, approved provider-independent baseline | `8be224d4ef5779f1743bdd7b4a53f731f31d564e` | Normative `SR-94-007`–`SR-94-011` invitation security/privacy requirements and dependent release gates |
...
| Negative and recovery test inventory | Test inventory §§4–6 | 95 globally namespaced scenarios: `INV-73-01`–`INV-73-17`, `VER-73-01`–`VER-73-10`, `CNS-73-01`–`CNS-73-08`, `DCL-73-01`–`DCL-73-13`, `DST-73-01`–`DST-73-08`, `CHG-73-01`–`CHG-73-13`, `RVK-73-01`–`RVK-73-16`, and `TRF-73-01`–`TRF-73-10` | **Draft scenarios, not executable fixtures.** The `VT-94` family selector is not a stable identifier; applicable implementation fixtures and open-issue evidence remain |
...
| CBD-73-AC12 | Each lifecycle state defines actor, preconditions, resulting state, message, notifications, audit event, and failure/recovery. | Invitation, change, revocation/removal, and transfer transition contracts in §§4, 10–12 | All 95 scenarios: `INV-73-01`–`INV-73-17`, `VER-73-01`–`VER-73-10`, `CNS-73-01`–`CNS-73-08`, `DCL-73-01`–`DCL-73-13`, `DST-73-01`–`DST-73-08`, `CHG-73-01`–`CHG-73-13`, `RVK-73-01`–`RVK-73-16`, `TRF-73-01`–`TRF-73-10` | Mapped at draft-rule level; `OI-73-008`–`OI-73-012` retain implementation gates |
...
| CBD-73-AC14 | Test inventory covers wrong recipient/channel, wrong space, invalid/expired/reused code, revoked consent, stale session, queued alert, unauthorized role change, cross-space isolation. | Test inventory §5 | Exact §5 required-case/hardening matrix; notably `VER-73-01`–`VER-73-02`, `VER-73-08`, `DCL-73-09`, `RVK-73-11`, `INV-73-02`, `INV-73-04`, `INV-73-09`–`INV-73-10`, `INV-73-15`–`INV-73-16`, `CNS-73-08`, `CHG-73-13`, `TRF-73-10`, `RVK-73-01`–`RVK-73-02`, `RVK-73-15`, `CHG-73-05`–`CHG-73-06`, `DST-73-07`–`DST-73-08` | Mapped; `OI-73-001` blocks wrong-recipient policy approval |
...
| `CBD-12-AC34` — one invited channel; verified control before acceptance | Specification §5; `VER-73-01`–`VER-73-10` | `OI-73-008`/`OI-73-012` |
...
| `OI-73-007` | **Confluence target and page registration.** No discoverable CBD-73 page or synchronization target exists. | Keep Confluence read-only. The repository remains the working source; do not publish this draft or audit branch. | Documentation owner / repository maintainer after approval and merge | Approved CBD-73 documents merged to `main`; four targets registered; synchronization completed; read-back parity verified | **Publication gate only; does not block repository review.** |
...
| `RV-73-003` | **Fixed in v0.2 draft** | Acceptance/removal rules terminalize any invitation issued before the end of the candidate account's prior membership; test inventory covers opened and unopened alternate-channel cases |
...
| `RV-73-026` | **Open publication gate — `OI-73-007`** | Repository-first workflow; target registration, post-merge synchronization, and read-back parity remain required |
...
| Confluence publication | Approval and merge to `main`, then `OI-73-007`: register targets, synchronize all four documents, and verify read-back parity. Confluence remains read-only before merge |
