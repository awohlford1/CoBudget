...
| Field | Value |
| --- | --- |
| Status | **Complete draft — implementation and specialist evidence remain open by design** |
...
| --- | --- | --- | --- | --- | --- | --- |
| FU-95-001 | P0 | External preview conflict; `RV-95-001`, `NT-92-*`, `EM-92-*`, `SR-94-044–054/109/122` | CBD-12 AC21, CBD-74 AC05, and CBD-58 are inconsistent with later approved CBD-92 ceilings | Apply the August 16, 2026 Product Owner decision: retain content-free push, SMS, and routine email, keep protected detail in-app, preserve purpose-specific email allowlists, and correct the three stale Jira surfaces without reopening CBD-92. | Current live-field diff; recorded Product Owner decision; reconciled acceptance criteria/descriptions; affected traceability and tests updated | Product conflict is resolved. CBD-12 AC21 and affected external-notification preview approval/implementation remain blocked until source correction; implementation must use the content-free ceiling. |
| FU-95-002 | P0 | Alert authority conflict; `RV-95-002`, CBD-12 AC19, CBD-72 §5.4 | CBD-74 AC02/09/11 and description are stale | Correct CBD-74 so users cannot create arbitrary alert rules or pause/disable eligible mandatory in-app alerts and cannot own trigger, threshold, cooldown, or dedup logic. Preserve only approved personal external delivery, quiet-hours/digest where supported, and presentation state. | Live CBD-74 comparison; corrected fields; CBD-11/CBD-71/CBD-72 inheritance trace; negative fixtures | CBD-74 execution against the stale criteria is blocked. Current fixed built-in behavior controls. |
| FU-95-003 | P1 | Accountability Partner wording; `RV-95-003`, `RI-93-001` | CBD-75 AC02 says “scoped support” | Replace only the ambiguous phrase with the approved comprehensive, fixed-field-boundary, financially read-only definition. Do not create a narrower Partner variant. | Live-field diff and terminology review against CBD-12/CBD-72 | CBD-75 copy approval is blocked; the approved comprehensive role still controls. |
| FU-95-004 | P0 | Stale deferred-product list; `RV-95-004` | CBD-76 AC06 contradicts CBD-76 description and approved CBD-72 | Remove multiple Co-owners, fixed Partner configuration, and exact built-in alert categories/triggers/thresholds from the deferred list. Retain only genuinely deferred items. | Live CBD-76 comparison; corrected AC; Product Owner confirmation that no settled decision was reopened | CBD-76 cannot start or be approved from its current stale boundary. |
| FU-95-005 | P3 | Source version metadata; `RV-95-005` and CBD-93 stale header text | CBD-73/74/75 cite CBD-71 v1.0; CBD-93 repository header describes an older published copy | Correct each item only in its authorized focused task. Do not create a broad cleanup change or edit Confluence before merge. | Focused repository/Jira diffs, merged versions, then post-merge Confluence comparison | Traceability is harder to audit; no current product behavior changes. |
...
| FU-95-029 | P2 | Post-merge publication and source parity | No CBD-95 Confluence pages exist; repository is the working source | After the exact repository documents merge to `main`, publish/synchronize the CBD-95 package to its approved Confluence targets and compare title, version, stable IDs, tables, limitations, hashes/links, and revision history. | Merge SHA; Confluence page IDs/versions; parity report with no unresolved material drift | Confluence publication is blocked until merge; local repository review is unaffected. |
| FU-95-030 | P2 | Final workflow evidence and links | CBD-95 is Ready, CBD-14 In Progress, CBD-72 In Progress, CBD-76 Ready and blocked by CBD-95 in the reviewed live state | With explicit user authorization and an immediate live refetch, attach exact repository/Confluence evidence, retain open implementation/specialist gates, correct only authorized stale fields, and transition issues only when their actual criteria are met. | Live before/after snapshots; approved version/SHA; issue links; comments; status transitions; no overwritten concurrent changes | No Jira mutation is authorized by this register. CBD-14/CBD-95/CBD-72 completion claims and CBD-76 start remain blocked. |
...
- for `RI-93-015`, the Product Owner approved tiered transparent notice
  ...
  inactivity history/countdown, speculation, or unsupported remedies.
  and
...
- for `RI-93-017`, the Product Owner approved a Private-MVP-only bounded honest
  ...
  training, and scenario evidence remain gated.
  and
- for `RI-93-018`, the Product Owner approved deletion of private
  ...
  described as remotely erasable.
  and
