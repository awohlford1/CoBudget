...
as a gap in a vendor's contract. `EV-102-175` is registered at Low confidence
even though it answers an open item, because the same page is demonstrably
wrong about Azure Private Link; the conclusion rests on `EV-102-012` with the
