...
| Governing provider decision | CBD-108 draft v0.69, In Progress; route B taken September 2, 2026 under CBD-103 §3.3; Private-MVP push has no provider to select (CBD-130 `PN-130-003`) |
...
| [CBD-108](https://cobudget.atlassian.net/browse/CBD-108) | `DEF-76-003` provider selection | In Progress; draft v0.69 selects six categories at `ELIGIBLE-PENDING-EVIDENCE` under route B (September 2, 2026), defers the observation tests to build, and records push as having no provider to select. | Product Owner approval of the exact package with its recorded deferrals, accepted risks, and residual-risk ownership. | Provider-dependent implementation waits for that approval; product semantics remain fixed under every route. |
